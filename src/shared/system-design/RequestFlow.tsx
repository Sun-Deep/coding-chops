...
const pointOnJourney = (
  ...
  if (progress < 0.3) {
    return between(client, server, progress / 0.3);
  }
  if (progress < 0.55) {
  ...
  return (
    <>
      ...
      {Array.from({ length: packetCount }, (_, index) => {
        const offset = overload ? index * 0.075 : 0;
        ...
        const packetOpacity = overload
          ? interpolate(index, [0, packetCount - 1], [1, 0.38], clamp)
          : 1;
        ...
        return (
          <div
            ...
            style={{
              ...
              background: overload ? theme.colors.red : theme.colors.blue,
              boxShadow: `0 0 24px ${overload ? theme.colors.red : theme.colors.blue}`,
            }}
          />
        );
      })}
